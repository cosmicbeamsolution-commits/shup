<template>
  <div>
    ...
    <section class="section is-main-section">
      <tiles v-show="shop.plan !== 'advanced' && shop.plan !== 'premium'" >
        <card-widget class="tile is-child" type="is-danger" icon="cash-usd" prefix="$" :number="salesCount" :link="'/ventas'" label="Ventas"/>
        <card-widget class="tile is-child" type="is-info" icon="tray-full" :number="itemsCount" :link="'/catalogo'" label="Catálogo"/>
        <card-widget class="tile is-child" type="is-success" icon="timeline-clock" :number="onlineCount" label="En línea"/>
      </tiles>
      <tiles v-show="shop.plan === 'advanced' || shop.plan === 'premium'" >
        <card-widget class="tile is-child" type="is-danger" icon="cash-usd" prefix="$" :number="salesCount" :link="'/ventas'" label="Ventas"/>
        <card-widget class="tile is-child" type="is-info" icon="tray-full" :number="itemsCount" :link="'/catalogo'" label="Catálogo"/>
        <card-widget class="tile is-child" type="is-success" icon="timeline-clock" :number="onlineCount" label="En línea"/>
        <card-widget class="tile is-child" type="is-success" icon="gesture-tap-button" :number="eventsCount" :link="'/'" label="Clics"/>
      </tiles>
      ...
      <div class="field">
        <card-component title="Eventos" icon="gesture-tap-button" class="animated fadeIn delay2">
      ...
      <div v-if="Object.keys(salesData).length" class="field">
        <card-component title="Últimas ventas" class="has-table has-mobile-sort-spaced animated fadeIn delay3">
          <table-component :data-actions="false" :data-set="salesData"/>
        </card-component>
      </div>
      <div class="field">
...
export default {
  ...
  created () {
    setTimeout(() => {
      this.$socket.emit('findConnected', this.shop.id)
    }, 1000)
    this.getSectionData()
  },
  methods: {

    ...
    <section class="section is-main-section">
      <div class="columns">
        <div v-for="(item, index) in data" :key="index" class="column is-clickable" @click="showPartial(index)">
          <card-widget class="tile is-child is-clickable" :class="{ 'is-active': activeIndex === index }" type="is-light" icon="clipboard-text-multiple-outline" :number="item.items_count" :label="item.formatted_date"/>
        </div>
      </div>
      <card-component v-if="Object.keys(partial).length">
        ...
        <table class="table is-striped is-fullwidth">
          <tr v-for="(item, index) in partial.items_sorted" :key="index" class="sales-item" :class="{ 'is-hidden': item.hidden }">
...
export default {
  ...
  methods: {
    ...
    search () {
      ...
      Object.keys(this.partial.items_sorted).forEach(e => {
        let hidden = false
        if (this.query.trim().length) {
          hidden = sanitize(e).indexOf(query) === -1
        }
        this.$set(this.partial.items_sorted[e], 'hidden', hidden)
      })
    },
    print () {
      window.print()
        ...
        if (res.data.length) {
          ...
          Object.keys(grouped).forEach(i => {
            ...
            this.data.push({
              ...
              items_sorted: this.$root.groupBy(row.flatMap(e => [...e.order]), 'name'),
              items_count: row.reduce((acc, e) => acc + e.order.length, 0),

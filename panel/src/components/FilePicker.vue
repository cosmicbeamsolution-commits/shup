  ...
  <div>
    <b-field class="file">
      <b-upload v-model="file" @input="upload" class="is-clickable" :accept="accept">
        ...
        <b-icon else icon="file-upload" type="is-info" size="is-large"/>
      </b-upload>
      <!--span class="file-name" v-if="file">{{ file.name }}</span-->
    </b-field>
    <div v-show="uploadPercent" class="is-upload-progress animated loop flash" :style="uploadPercent?'background: linear-gradient(to right,#00d1b2 '+uploadPercent+'%,transparent '+uploadPercent+'%)':''"/>
  </div>
</template>

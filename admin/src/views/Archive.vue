<template>
  <div>
    ...
    <hero-bar>
      <b-icon icon="sale" size="is-large"/>
      Ventas
      ...
      <card-component v-if="partial.sales">
        ...
        <b-table
          ...
          <template slot="detail" slot-scope="props">
            <span class="tag is-tag-spaced" v-for="(item, index) in props.row.products_sorted" :key="index" :style="{ backgroundColor: item[0].color }">
      ...
      axios.post('/sales/sort', {
        ...
        if (res.data.length) {
          ...
          Object.keys(grouped).forEach(i => {
            ...
            row.map(e => {
              e.products_sorted = this.$root.groupBy(e.order, 'name')
              return e
            })
            this.data.push({
              date: i,

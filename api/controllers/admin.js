...
module.exports = {
  ...
  users:  (req, res) => {
    let from = moment().subtract(req.body.days, 'days').format()
    if (req.body.days <= 1) {
      from = moment().subtract(parseInt(req.body.days * 24), 'hours').format()
    }
    if (req.body.days <= 0.041666667) {
      from = moment().subtract(parseInt(req.body.days * 24 * 60), 'minutes').format() 
    }
    req.app.db.collection('events').aggregate([
      { $match: {'history.created': {"$gte": from}}},
      { $unwind: '$history'},
      { $match: {'history.created': {"$gte": from}}},
    ...
    req.app.db.collection('proveedores').findOneAndUpdate(
    {
      ...
      return emailClient.send({
        ...
        data: {
          ...
          linkText: 'Conocer más acerca de Geotiendas',
          tag: 'cliente'
        },
        templatePath:path.join(__dirname,'/../email/template.html')
      }).then(function(){
  ...
  approve: (req, res) => {
    ...
    if (req.body.plan === 'free') {
      ...
      data.message = '¡Fuiste aprobado para formar parte de Geotiendas! Seguí el enlace para obtener acceso a tu panel de proveedor.',

...
module.exports = {
  ...
  create: (req, res) => {
    ...
    const superemails = [
      'geotiendascom@gmail.com',
      'overlemonsoft@gmail.com',
      'carrizomarcia@gmail.com',
      'mafrith@gmail.com'
    ]
    var password = req.body.password
    ...
    req.app.db.collection('shops').countDocuments({
      ...
      if (count && !superemails.includes(req.body.email)) {
        return res.json({
          status: 'danger',
      ...
      bcrypt.hash(password, saltRounds, function (err, hash) {
        ...
        },
        {
          ...
          return emailClient.send({
            to: doc.value.email,
            subject: `Validación de tienda ${doc.value.shop}`,
            data: {
...
Gracias por confiar en nosotros y te damos la bienvenida a esta hermosa comunidad de emprendedores que cada día crece más.<br>`,
  ...
  basic: (req, res) => {
    ...
    req.app.db.collection('shops').findOne(find, (err, shop) => {
      if (!shop) {
        return res.json({
          id: id
        })
